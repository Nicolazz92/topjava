...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                ...
                .addValue("time", Timestamp.valueOf(meal.getDateTime()))
        ...
        if (meal.isNew()) {
            ...
            namedParameterJdbcTemplate.update(
                    "UPDATE meals SET time=:time, description=:description, calories=:calories WHERE id=:id"
                    , map);
        }
        return meal;
    }
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? ORDER BY time DESC", ROW_MAPPER, userId);
    }

    @Override
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? AND time BETWEEN ? AND ? ORDER BY time DESC",
